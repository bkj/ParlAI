...
class BiEncoderRankerAgent(TorchRankerAgent):
    ...
    def __init__(self, opt, shared=None):
        print('!! init BiEncoderRankerAgent', file=sys.stderr)
        ...
        def _save_vecs():
            ...
            print('!! trying to save', file=sys.stderr)
            uuid = str(uuid4())
            torch.save(self._all_ctx_txt, './experiments/bi_encoder/v0/vecs/_all_ctx_txt.%s' % uuid)
            torch.save(self._all_ctx_emb, './experiments/bi_encoder/v0/vecs/_all_ctx_emb.%s' % uuid)
            torch.save(self._all_cnd_txt, './experiments/bi_encoder/v0/vecs/_all_cnd_txt.%s' % uuid)
            torch.save(self._all_cnd_emb, './experiments/bi_encoder/v0/vecs/_all_cnd_emb.%s' % uuid)
            print('!! done saving', file=sys.stderr)
        
        atexit.register(_save_vecs)
        # <<
        ...
        if shared:
            ...
            if 'vocab' in (self.opt['candidates'], self.opt['eval_candidates']):
                ...
                for ind in range(1, len(self.dict)):
                    ...
                    cands.append(txt)
                    vecs.append(
                        self._vectorize_text(txt, add_start=True, add_end=True, truncate=self.label_truncate)
                    )
    ...
    def vectorize_fixed_candidates(self, cands_batch):
        ...
        return [self._vectorize_text(cand, add_start=True, add_end=True, truncate=self.label_truncate)
            for cand in cands_batch]

    def encode_candidates(self, padded_cands):
        token_idx_cands, segment_idx_cands, mask_cands = to_bert_input(padded_cands, self.NULL_IDX)
    ...
    def score_candidates(self, batch, cand_vecs, cand_encs=None):
        ...
        token_idx_ctxt, segment_idx_ctxt, mask_ctxt = to_bert_input(batch.text_vec, self.NULL_IDX)
        ...
        # >>
        self._all_ctx_txt += [xx['text'] for xx in batch['observations']]
        self._all_ctx_emb.append(embedding_ctxt.cpu().numpy())
        # <<
        ...
        elif len(cand_vecs.size()) == 3:
            csize = cand_vecs.size()  # batchsize x ncands x sentlength
            cands_idx_reshaped = cand_vecs.view(csize[0] * csize[1], csize[2])
            token_idx_cands, segment_idx_cands, mask_cands = to_bert_input(cands_idx_reshaped, self.NULL_IDX)
            
            _, embedding_cands = self.model(None, None, None, token_idx_cands, segment_idx_cands, mask_cands)
            
            embedding_cands = embedding_cands.view(csize[0], csize[1], -1)  # batchsize x ncands x embed_size
            embedding_cands = embedding_cands.transpose(1, 2)  # batchsize x embed_size x ncands
            
            # >>
            self._all_cnd_txt += [xx['label_candidates'] for xx in batch['observations']]
            self._all_cnd_emb.append(embedding_cands.cpu().numpy())
            # <<
            
            embedding_ctxt  = embedding_ctxt.unsqueeze(1)  # batchsize x 1 x embed_size
            
            scores = torch.bmm(embedding_ctxt, embedding_cands)  # batchsize x 1 x ncands
